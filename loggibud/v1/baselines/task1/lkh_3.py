...
def solve(
    ...
    convert_instance_file(instance, params, auxiliary_file_names)

    logger.info("Calling LKH external solver")
    solve_lkh(instance, params, auxiliary_file_names)

    logger.info("Reading result from output files")
    solution = read_solution(instance, params, auxiliary_file_names)
...
def convert_instance_file(
    instance: CVRPInstance,
    params: LKHParams,
    auxiliary_file_names: Dict[str, str],
) -> None:
    """
...
def solve_lkh(instance: CVRPInstance, params: LKHParams, auxiliary_file_names: Dict[str, str]) -> None:
    """Call the C solver and generate output files"""

    arguments = ("./loggibud/v1/baselines/task1/LKH", auxiliary_file_names["input_par_file"])
    popen = subprocess.Popen(arguments, stdout=subprocess.PIPE)
    popen.wait()  # run the solver in the background


def read_solution(instance: CVRPInstance, params: LKHParams, auxiliary_file_names: Dict[str, str]) -> CVRPSolution:
